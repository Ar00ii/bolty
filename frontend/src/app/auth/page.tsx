    ...
    <div className="mt-2 space-y-2">
      <div className="flex gap-1 items-center">
        {[1, 2, 3].map((n) => (
          <div
            key={n}
            className={`h-1 flex-1 rounded-full transition-colors duration-300 ${n <= str ? meta.color : 'bg-zinc-800'}`}
          />
        ))}
        {meta.label && (
          <span
            className={`text-xs ml-1 font-light ${str === 1 ? 'text-red-400' : str === 2 ? 'text-yellow-400' : 'text-green-400'}`}
          >
            {meta.label}
          </span>
        )}
...
const BRAND_FEATURES = [
  {
    icon: Code2,
    title: 'Publish & monetize repos',
    desc: 'Earn ETH from your locked repositories.',
  },
  { icon: Bot, title: 'Deploy AI agents', desc: 'List tools and bots on the marketplace.' },
  {
    icon: MessageSquare,
    title: 'Global dev community',
    desc: 'Real-time chat with devs worldwide.',
  },
    ...
    <div className="flex flex-col px-8 py-10 h-full justify-center">
      ...
      <div className="grid grid-cols-2 gap-2 mb-6">
        {BRAND_STATS.map((s) => (
          <div
            key={s.label}
            className="border border-white/[0.06] rounded-xl p-2.5 text-center"
      ...
      {/* Testimonial */}
      <div
        className="border border-white/[0.06] rounded-xl p-3"
      ...
      >
        ...
        <p className="text-[10px] text-zinc-400 leading-relaxed mb-2">
          &ldquo;{BRAND_TESTIMONIAL.text}&rdquo;
        </p>
        <div className="flex items-center gap-1.5">
          <div
            className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-light text-monad-400 flex-shrink-0"
    ...
    <div className="flex items-center gap-2 mb-5">
      {[1, 2].map((s) => (
        <React.Fragment key={s}>
          <div className="flex items-center gap-1.5">
            <div
              className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-light transition-all ${
                s < step
            ...
            <span
              className={`text-xs ${s === step ? 'text-zinc-300 font-light' : 'text-zinc-600'}`}
            >
              {s === 1 ? 'Account' : 'Profile'}
            </span>
          </div>
          {s < 2 && (
            <div
              className={`flex-1 h-px transition-all ${s < step ? 'bg-monad-500' : 'bg-zinc-800'}`}
            />
          )}
        </React.Fragment>
      ))}
...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden px-4 py-8">
      ...
      {/* Centered card */}
      <div
        className="relative z-10 w-full max-w-4xl flex rounded-2xl overflow-hidden"
        style={{
      ...
      >
        {/* Left: Brand panel */}
        <div
          className="hidden lg:flex lg:w-[42%] flex-col relative"
          style={{
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                'radial-gradient(ellipse at center, rgba(131,110,249,0.05) 0%, transparent 65%)',
            }}
          />
        ...
        <div className="flex-1 flex flex-col px-8 py-10 overflow-y-auto" style={{ minWidth: 0 }}>
              ...
              {twoFactorPending ? (
                ...
                <span>
                  ...
                  <button
                    ...
                    className="text-monad-400 hover:text-monad-300 font-light transition-colors"
                  >
                    Sign up
                  </button>
                </span>
                ...
                'Step 2 of 2 — Optional profile details'
              ) : tab === 'forgot' ? (
                <span>
                  {'Remember it? '}
                  <button
          ...
          {!twoFactorPending && tab === 'register' && <RegisterProgress step={regStep} />}
          ...
          {twoFactorPending && (
            <form onSubmit={handle2FAVerify} className="space-y-4">
              <div
                className="rounded-xl border border-monad-500/20 p-4"
                style={{ background: 'rgba(131,110,249,0.05)' }}
              >
                <p className="text-xs text-zinc-400 leading-relaxed">
                  Two-factor authentication is enabled on this account. We sent a 6-digit code to
                  your email. It expires in 10 minutes.
                </p>
              </div>
              <div>
                <label className="block text-xs font-light text-zinc-400 mb-1.5">
                  Verification code
                </label>
                <input
                  type="text"
              ...
              <div className="flex gap-3 bg-green-500/10 border border-green-500/20 rounded-xl px-4 py-4">
                ...
                <div>
                  ...
                  <p className="text-zinc-400 text-xs mt-0.5">
                    If an account with that email exists, you will receive a link within a few
          ...
          {!twoFactorPending && tab === 'register' && regStep === 2 && (
            <form onSubmit={handleRegisterStep2} className="space-y-4">
              ...
              >
                <p className="text-xs text-zinc-500 leading-relaxed">
                  Help us personalize your experience. Gender is required; everything else is
                  optional.
                </p>
              </div>
              <div>
              ...
              <Field
                ...
                type="text"
                value={regOccupation}
                onChange={setRegOccupation}
                placeholder="e.g. Software Engineer, Student, Freelancer..."
                hint="Optional"
              />

              <div
                className="rounded-xl border border-monad-500/15 p-4"
                style={{ background: 'rgba(131,110,249,0.04)' }}
              >
                <p className="text-xs font-light text-monad-400 mb-2">
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={item} className="flex items-center gap-2 text-xs text-zinc-400">
                      <CheckCircle
                        className="w-3 h-3 text-monad-400 flex-shrink-0"
                        strokeWidth={2}
                      />
                      {item}
                    </div>
