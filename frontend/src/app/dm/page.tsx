...
function Avatar({
  ...
  return (
    <div
      className={`${sz} rounded-full flex-shrink-0 flex items-center justify-center text-monad-400 font-light text-xs`}
      style={{
        background: 'var(--brand-dim, rgba(131,110,249,0.12))',
...
export default function DmPage() {
  ...
  useEffect(() => {
    ...
    socket.on(
      'conversation',
      ({ peerId, messages: msgs }: { peerId: string; messages: DMMessage[] }) => {
        ...
        <div className="w-64 hidden md:flex flex-col gap-3 flex-shrink-0">
          ...
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-2">
              <span className="text-sm font-light" style={{ color: 'var(--text)' }}>
                Messages
              </span>
              <div
          ...
          {showNewDm && (
            <div className="flex gap-2 px-1">
              ...
              />
              <button onClick={startNewDm} className="btn-neon text-xs py-1.5 px-3">
                Go
              </button>
            </div>
          )}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-light truncate" style={{ color: 'var(--text)' }}>
                      @{c.user.username || c.user.id.slice(0, 8)}
                    </span>
                    {c.unread > 0 && (
                      <span className="text-xs bg-monad-500 text-white rounded-full w-4 h-4 flex items-center justify-center font-light flex-shrink-0">
                        {c.unread}
                      </span>
                    )}
                  </div>
          ...
          {!activePeer ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
              <div className="w-12 h-12 rounded-2xl bg-monad-500/10 border border-monad-500/20 flex items-center justify-center mb-4">
                <svg
                  className="w-6 h-6 text-monad-400"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth="1.5"
              ...
              <h3 className="text-sm font-light mb-1" style={{ color: 'var(--text)' }}>
                Direct Messages
              </h3>
                ...
                {messages.map((msg) => {
                  ...
                  return (
                    <div key={msg.id} className={`flex gap-2.5 ${isMe ? 'flex-row-reverse' : ''}`}>
                      {!isMe && (
                        <Avatar name={msg.senderUsername} url={msg.senderAvatar} size={7} />
                      )}
                      <div
                      ...
                      >
                        ...
                        <div
                          className={`px-3.5 py-2.5 text-sm leading-relaxed break-words ${
                            isMe
                              ? 'bg-monad-500 text-white rounded-2xl rounded-tr-md'
