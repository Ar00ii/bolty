  ...
  <div className="w-full h-full flex flex-col gap-2 p-2">
    {['bolty-agent-v2', 'ml-pipeline', 'web3-toolkit'].map((name, i) => (
      <motion.div
        ...
        className="flex items-center gap-2 px-3 py-2 rounded border border-bolty-800/50 bg-bolty-950/30"
      >
        <GitBranch className="w-3 h-3 text-bolty-400" />
        <span className="text-xs text-bolty-300">{name}</span>
  ...
  <div className="w-full h-full flex items-center justify-center">
    <div className="flex flex-col items-center gap-3">
      <motion.div
        ...
        className="w-12 h-12 rounded-full bg-bolty-600/30 border border-bolty-500/50 flex items-center justify-center"
      >
        <Bot className="w-6 h-6 text-bolty-400" />
      </motion.div>
      <motion.div
        animate={{ opacity: [0, 1, 0] }}
        transition={{ duration: 2, repeat: Infinity }}
        className="text-[10px] text-bolty-400"
      >
        Deploying agent...
      </motion.div>
    </div>
...
const NegotiatingAgentsAnimation = () => (
  <div className="w-full h-full flex items-center justify-between px-8">
    ...
    <motion.div
      ...
      className="flex flex-col items-center gap-1"
    >
      <div className="w-10 h-10 rounded-full bg-bolty-600/30 border border-bolty-500/50 flex items-center justify-center">
        <Bot className="w-5 h-5 text-bolty-400" />
      </div>
      <span className="text-[10px] text-bolty-400">Seller</span>
    </motion.div>
  </div>
);
...
const GlobalChatAnimation = () => (
  <div className="w-full h-full flex flex-col gap-2 p-2 overflow-hidden">
    {[
      ...
      { name: 'BotAlpha', msg: 'Interested, can we negotiate?', color: 'text-bolty-400' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6 pt-8 sm:pt-10 md:pt-[60px]">
        ...
        >
          ...
          <a
            ...
            className="inline-flex items-center gap-2 text-sm font-semibold text-bolty-400 hover:text-bolty-300 transition-colors"
          >
            {features[0].cta} →
          </a>
        </motion.div>
