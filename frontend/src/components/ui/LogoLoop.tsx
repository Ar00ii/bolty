...
type ImgLogoItem = { src: string; srcSet?: string; sizes?: string; width?: number; height?: number; alt?: string; title?: string; href?: string };
...
export const LogoLoop = memo(({
  ...
  const updateDimensions = useCallback(() => {
    ...
    if (isVertical) {
      ...
      if (sh > 0) {
        ...
        const viewport = containerRef.current?.clientHeight ?? ph ?? sh;
  ...
  const renderLogoItem = useCallback((item: LogoItem, key: string) => {
    ...
    const content = isNode
      ...
      : <img src={(item as ImgLogoItem).src} srcSet={(item as ImgLogoItem).srcSet} alt={(item as ImgLogoItem).alt ?? ''} loading="lazy" decoding="async" draggable={false} />;
    const label = isNode ? ((item as NodeLogoItem).ariaLabel ?? (item as NodeLogoItem).title) : ((item as ImgLogoItem).alt ?? (item as ImgLogoItem).title);
