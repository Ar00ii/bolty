        ...
        <header className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tight text-white leading-none">
                {title}
              </h1>
            </div>
          </div>
            ...
            <nav className="space-y-1">
              {sections.map((s) => (
                <a
                  key={s.id}
                  href={`#${s.id}`}
                  className="block text-xs text-zinc-400 hover:text-white py-1.5 border-l border-white/[0.06] hover:border-bolty-400/40 pl-3 transition-colors"
                >
                  {s.title}
                </a>
              ))}
          ...
          <main className="min-w-0 space-y-10">
            {sections.map((s) => (
              <section key={s.id} id={s.id} className="scroll-mt-20">
                <h2 className="text-lg sm:text-xl font-semibold text-white mb-3 tracking-tight">
                  {s.title}
                </h2>
                <div className="text-sm text-zinc-400 leading-relaxed font-semibold space-y-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:space-y-1.5 [&_a]:text-bolty-300 [&_a:hover]:text-bolty-200">
                  {s.body}
                </div>
              </section>
            ))}
