  ...
  return (
    <div className="w-full">
      <div className="relative flex flex-col items-start">
        {label && (
          <label className="text-xs font-mono text-zinc-500 mb-1.5 block">{label}</label>
        )}
        <div className="relative w-full">
          <Input
            ...
            value={query}
            onChange={e => { setQuery(e.target.value); setSelectedAction(null); }}
          ...
          <AnimatePresence>
            {isFocused && result && !selectedAction && (
              <motion.div
                ...
                style={{ background: 'rgba(18,18,21,0.98)', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}
                variants={container} initial="hidden" animate="show" exit="exit"
              >
                <motion.ul className="max-h-56 overflow-y-auto">
                  {result.actions.length === 0 && (
                    <li className="px-4 py-3 text-xs text-zinc-600 font-mono text-center">No results</li>
                  )}
                  {result.actions.map(action => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 ml-2 flex-shrink-0">
                        {action.short && <span className="text-xs text-zinc-600 font-mono">{action.short}</span>}
                        {action.end && (
                ...
                {result.actions.length > 0 && (
                  <div className="px-4 py-2 flex items-center justify-between text-xs text-zinc-700 font-mono"
                    style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                    <span>{result.actions.length} result{result.actions.length !== 1 ? 's' : ''}</span>
